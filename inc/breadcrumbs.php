...
if ( ! function_exists( 'medical_breadcrumbs' ) ) {
    function medical_breadcrumbs( $args = array() ) {
        if ( is_front_page() ) {
            return;
        }
        ...
        $defaults  = array(
            ...
            'home_title'          => esc_html__( 'Home', 'medical' )
        );
        $args      = apply_filters( 'medical_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
        ...
        */
        ...
        $html .= '<a class="bread-link s-text16 bread-home" href="' . esc_url( get_home_url('/') ) . '" title="' . esc_attr( $args['home_title'] ) . '">'.esc_html__( 'Home', 'medical' ).' <span class="lnr lnr-arrow-right"></span></a>';
        // Post
        if ( is_singular( 'post' ) ) {
            ...
            $html .= '<span class="s-text17 bread-search">'.esc_html__('Search results for:','medical') . get_search_query() . '</span>';
        } elseif ( is_404() ) {
            $html .= '<span>' . esc_html__( 'Error 404', 'medical' ) . '</span>';
        } elseif ( is_home() ) {
            $html .= '<span class="s-text17">' . get_the_title( get_option( 'page_for_posts' ) ) . '</span>';
        }
        $html .= '</div>';
        $html = apply_filters( 'medical_breadcrumbs_filter', $html );
