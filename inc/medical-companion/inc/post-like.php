...
add_action( 'wp_enqueue_scripts', 'medical_sl_enqueue_scripts' );
function medical_sl_enqueue_scripts() {

	wp_enqueue_script( 'medical-post-likes', MEDICAL_COMPANION_DIR_URL . '/js/post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'medical-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'medical' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_medical_process_simple_like', 'medical_process_simple_like' );
add_action( 'wp_ajax_medical_process_simple_like', 'medical_process_simple_like' );
function medical_process_simple_like() {
	// Medical
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'medical' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	$output = '';
	$nonce = wp_create_nonce( 'simple-likes-nonce' ); // Medical
	if ( $is_comment == 1 ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'medical' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'medical' );
		$icon = $icon_empty;
	}
...
// User Profile List
add_action( 'show_user_profile', 'medical_show_user_likes' );
add_action( 'edit_user_profile', 'medical_show_user_likes' );
function medical_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'medical' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'medical' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
